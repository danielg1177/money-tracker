...
- **PHPUnit:** sqlite `:memory:` via `.env.testing` (process env / `.env.testing`); `phpunit.xml` does not hardcode DB credentials
...
| `2026_06_04_193747_add_income_additions_to_debts_table` | Adds nullable `income_additions` JSON on `debts` (income attached to an existing debt) |
| `2026_06_04_194211_add_is_loan_receipt_to_transactions_table` | Adds `is_loan_receipt` boolean on `transactions` |
...
| `2026_05_05_201653_add_advance_fund_id_to_transactions_table` | Nullable `advance_fund_id` FK on `transactions` → `funds.id`. Source chains `nullOnDelete()` **before** `constrained('funds')`. **Railway production** constraint `transactions_advance_fund_id_foreign` is restrict (no SET NULL) — deleting a referenced fund 500s |
...
| `2026_07_14_194817_add_debt_payment_benefit_columns_to_transactions_table` | Adds `is_debt_payment_benefit` bool and `debt_payment_income_id` FK (`nullOnDelete`) for creditor benefit expenses linked to debt-payment income |
...
| `is_debt_payment_benefit` | boolean | default false — creditor expense linked to debt-payment income |
...
| `income_additions` | json nullable | Income attached to an existing debt (`income_debt_mode=existing`): `[{amount, date, transaction_id?}]`; principal `amount` is unchanged |
...
| `user_id` | bigint FK | → `users.id` |
| `type` | varchar | `allocation` \| `borrow` \| `repayment` \| `initial_value` \| `closeout_allocation` \| `advance_settlement` \| `savings_sweep` \| `manual_override` (signed delta; fund history only) |
| `amount` | decimal(15,2) | |
| `description` | text nullable | |
