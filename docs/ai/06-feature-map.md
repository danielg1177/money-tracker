...
**Debt payment (`DebtService::payDebt`):** Creates two transactions — an expense for the debtor, and an income for the creditor (if `creditor_id` is not null / not a fund debt). Rejects pending split debts. External debts (`creditor_id` null) still cap payment at remaining balance; **in-family overpayment** zeros the paid debt and applies the excess via **`applyInterFamilyPairNet`** (swapped direction; merges into an existing reverse debt when present). Accepts optional `transaction_date` so debt payments use the user-selected date instead of always defaulting to today. **`GET /debts/{debt}/payments`** lists one entry per pay action for those debts by excluding the mirror **income** row when a matching **expense** exists (paired on `debt_id`, date, amount, `paid_by_user_id`, and `created_at`). Payment rows now include `split_breakdown` when applicable so history can show each participant’s paid share for split repayments. A synthetic `'initial_value'` entry is appended at the end showing the debt's original amount and creation date. In `Debts.vue` payment history rendering, repayment rows (debtor expense or creditor income) display as **green negative** values because they reduce debt, while closeout contribution additions display as **red positive** values; closeout-tagged payment rows include viewer-direction copy ("paid by you" vs "received by you").
