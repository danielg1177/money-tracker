...
- `family_category_totals` / `family_category_transactions`: household overlay (same exclusion flags as viewer totals; split expenses counted **once at full amount**; rows include `user_id` / `user_name`). Omitted when the preference is off. Does not change `member_balances`, `rule_preview`, `fund_movements`, or `title_savings`. When the preference is on, `debt_repayments` also includes `family_debt_paid`.
