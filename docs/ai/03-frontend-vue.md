...
| `/bank-connections` | `BankConnections.vue` | `requiresAuth` — Plaid Link (`financekit_supported` on link token when enabled for Apple Card / Wallet where Plaid allows), pending-import banner, **Sync all banks this month** / **last month** (`POST /plaid/sync-month`, `POST /plaid/sync-last-month` when any item linked) plus per-item sync-this-month / sync-last-month, calibrate navigation, disconnect; note that **pending bank charges are skipped until they post**; short Apple Card note under Connect |
