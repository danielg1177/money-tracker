...
- **Your expenses / your income categories:** Viewer-scoped monthly totals grouped by category (expense totals in red, income in green; **split bills use only your portion** — same rules as **`GET /month-summary` `category_totals`**). Tracked **debt repayments** you pay (solo or your split share) roll up as a **Debt payments** line (`category_id` sentinel **-1**), not under the posting category; **Debt repayments** below still lists each repayment for context. Each section sorts categories by amount (highest first), shows the **top four** by default with a **full-width** control to reveal the rest when there are more, then a **Total expenses** / **Total income** summary row for **all** categories in that section (not only the visible four). A short note under **Your Expenses** explains how this ties to **Projected closeout → Expenses** vs hard-close-generated ledger lines.
...
- **Projected Closeout / Closeout Results:** Dry-run preview of the month's fund allocation rules with basis (gross income, expenses, remaining). Each rule shows **`net_after_advances`** (amount after netting month advances tagged `advance_fund_id` toward that fund—**negative** when advances exceed the nominal rule line); subtitle lines clarify **rule allocation minus advances** for fund destinations
