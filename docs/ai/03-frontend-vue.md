...
The form watches `transaction_date` and posts `/closeout/status` for that month. If the family month is hard-closed, or the signed-in user has soft-closed that month, it shows a closed-month banner and blocks submit/edit controls while leaving the date input usable so the user can move the transaction to an open month. Backend `ClosedMonthGuard` is still authoritative and also blocks affected split participants or mirrored debt-payment creditors whose month is closed.
