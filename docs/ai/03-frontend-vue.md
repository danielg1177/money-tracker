...
- **Native `type="date"` on iOS:** Global base styles constrain **`date` / `datetime-local` / `month` / `time`** to **`width/max-width: 100%`**, **`min-width: 0`**, plus WebKit **`::-webkit-datetime-edit-fields-wrapper { max-width: 100% }`**. Bottom sheets (`AppNav` transaction modal, `Transactions.vue` edit modal, `Debts`/`Funds` modals) add **`min-w-0`**, **`overflow-x-hidden`**, and padded areas **`min-w-0 max-w-full`**. **`TransactionForm`** date row and **`Transactions`** custom-range dates use **`min-w-0`** wrappers; custom range stacks **`flex-col` on small screens** so two date pickers + Apply do not overflow.
