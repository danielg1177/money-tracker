...
**Inter-family debt cards:** For debts between two family members (`creditor_id` present), the debt page hides the **Original** amount on the card and in the history modal summary line; only the **Remaining** balance summary is shown there.
...
- **Close status header:** Lock icon indicating hard-closed (amber), all members soft-closed (blue), or open (gray outline). **Hard Close** is shown when the month is not hard-closed, every member has soft-closed, and the signed-in user has **`can_manage_family`** (`head_of_household` or `is_admin`), matching `MonthCloseoutController` authorization. When the viewed month is already hard-closed and the user has `can_manage_family`, the header shows an **Undo Hard Close** destructive action that prompts a native confirmation dialog before posting to `/closeout/undo-hard-close`.
...
The form watches `transaction_date` and posts `/closeout/status` for that month. If the family month is hard-closed, or the signed-in user has soft-closed that month, it blocks that date selection and immediately reverts to the last allowed date (showing a validation message), so users cannot keep a closed-month date selected in the picker. Backend `ClosedMonthGuard` is still authoritative and also blocks affected split participants or mirrored debt-payment creditors whose month is closed.
