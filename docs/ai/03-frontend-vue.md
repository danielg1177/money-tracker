...
- **Close status header:** Lock icon indicating hard-closed (amber), all members soft-closed (blue), or open (gray outline). **Hard Close** is shown when the month is not hard-closed, every member has soft-closed, and the signed-in user has **`can_manage_family`** (`head_of_household` or `is_admin`), matching `MonthCloseoutController` authorization
