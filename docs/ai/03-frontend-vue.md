...
Shared repayment picker for Plaid import review: used on **single-row confirm** (`formFor(row)` + import amount) and **split lines** (`PlaidImportSplitLineOptions`). Loads `GET /transactions/repayable-expenses` when the toggle is enabled.
