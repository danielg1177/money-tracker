...
- **Split balances (this month):** Placed after **Your Income** — net shared-expense IOUs vs each member (**`member_balances`**); excludes repayment and closeout split parents; omits counterparties who net to zero or had no qualifying split activity vs you. Each member row also shows source subtotals from split transactions **you created** vs **they created**, with directional signs/colors per source row (**green positive = they owe you**, **red negative = you owe them**) and a History button for each source that opens a transaction-detail bottom sheet. In that sheet, rows are grouped by category with a category header (including icon when available), then ordered by transaction date within each category. If a row has no description, the row label falls back to the category name.
