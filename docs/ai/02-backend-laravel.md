...
- `handleRepaymentForTransaction(Transaction, array $validatedData): void` — when `is_repayment_mode` is false, calls `deleteRepaymentLinks` only; when true, validates `repayment_for_user_id` (same family), replaces links via delete + `createRepaymentLinks` with `repayment_links`. Called from `TransactionController::store` / `update`, `PlaidImportController::confirm` / `confirmSplit` (per line); `deleteRepaymentLinks` runs before `TransactionController::destroy`.
