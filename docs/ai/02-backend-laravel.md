...
- `ledgerLinkCandidates(Request, PlaidPendingImport)` — `GET …/ledger-candidates`; owner-only; non-transfer pending only; JSON candidate ledger rows for manual linking (see `PlaidMatchingService::findLedgerLinkCandidatesForPendingImport`).
...
- `findLedgerLinkCandidatesForPendingImport(PlaidPendingImport, familyId, dayRadius?, limit?)` — wider ±`dayRadius` date window (default 45) than calibration auto-match; same amount (±0.01) and `suggested_type`; returns scored ledger rows for import-review linking UI.
- `canLinkPendingImportToLedger(PlaidPendingImport, Transaction, maxDateDriftDays?)` — validates unlinked ledger row, type/amount alignment, date within default **60** days of the pending row’s date.
