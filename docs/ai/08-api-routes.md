...
| GET | `/plaid/pending-imports` | `PlaidImportController::index` | Default JSON `{ pending, transfers, auto_created, dismissed }` (`pending` / `transfers` split by `is_transfer` among `status=pending`; **`auto_created`** = `status=auto_created` **and** `reviewed_at` null, eager-loads `suggestedCategory`, `plaidItem`, and **`transaction`** with `user`, `category`, `splits.user`, `debt.*`, `advanceFund`, `fund`, `paidByUser` for the **On your books** panel; `dismissed` includes only `dismiss_source='auto'` rows with `reviewed_at` null, eager-loads `plaidItem`). With `?count_only=1`, JSON `{ count, auto_created_count, dismissed_count }` — lightweight for nav badge |
