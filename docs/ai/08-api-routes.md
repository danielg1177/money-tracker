...
| GET | `/plaid/pending-imports/{pendingImport}/sweep-candidates` | `PlaidImportController::sweepCandidates` | JSON array (max 20) of unlinked `savings_sweep` `FundMovement` candidates for the import owner’s family — `{ id, amount, description, date, fund_name }`, ±60 days on `created_at`, ordered by amount proximity; `[]` when user has no `family_id`; owner-only |
| POST | `/plaid/pending-imports/{pendingImport}/link-to-sweep` | `PlaidImportController::linkToSweep` | Body: `{ fund_movement_id: int }` (required, exists); owner-only; `status` must be `pending` or `auto_linked`; links Plaid id on the sweep movement, pending `confirmed` + `fund_movement_id`; `422` if wrong type / already linked; `403` if movement fund not in user’s family |
