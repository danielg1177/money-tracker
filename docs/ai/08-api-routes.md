...
| GET | `/plaid/pending-imports` | `PlaidImportController::index` | Default JSON `{ pending, transfers, auto_created, dismissed }` (`pending` / `transfers` split by `is_transfer` among `status=pending`; `auto_created` rows include `suggestedCategory`, `plaidItem`, `transaction.category`; `dismissed` includes only `dismiss_source='auto'` rows with `reviewed_at` null, eager-loads `plaidItem`). With `?count_only=1`, JSON `{ count, auto_created_count, dismissed_count }` — lightweight for nav badge |
...
|| POST | `/plaid/pending-imports/{pendingImport}/approve-auto-created` | `PlaidImportController::approveAutoCreated` | Approves an `auto_created` row, reinforcing the merchant rule via `learnFromConfirmation`; `204`; owner-only |
|| POST | `/plaid/pending-imports/{pendingImport}/correct-auto-created` | `PlaidImportController::correctAutoCreated` | Updates the linked transaction's `category_id`, `type`, `fund_id`, `advance_fund_id`, `is_non_necessity` and retrains the merchant rule; body: `{ category_id, type, fund_id?, advance_fund_id?, is_non_necessity? }`; returns updated transaction JSON; `422` if not `auto_created` or no linked transaction |
