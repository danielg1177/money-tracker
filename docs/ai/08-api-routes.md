...
| POST | `/plaid/pending-imports/{pendingImport}/undo-confirm` | `PlaidImportController::undoConfirm` | Owner-only; requires `status=confirmed` with linked `transaction`; `ClosedMonthGuard::assertTransactionMutationOpen` on the transaction; deletes repayment links + ledger row via `TransactionService::deleteTransaction`, resets import to `status=pending` with `transaction_id` null; JSON `{ success: true, pending_import }`; `422` if not confirmed / no transaction / closed month |
