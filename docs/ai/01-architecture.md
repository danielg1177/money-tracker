...
| FundMovement | Per `fund_id` + `user_id`; types include `initial_value` (fund start), `allocation`, `borrow`, `repayment`, `closeout_allocation`, `advance_settlement` (advance fund payout at close), `savings_sweep` (manual sweep to external savings; no transaction) |
