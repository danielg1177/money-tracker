...
- `TransactionRepaymentService` — expense-repayment linking (family: mirror expense rows + `transaction_repayment_links`; external: `is_external_repayment` links with no mirror)
- `FundService` — borrow from fund, repay fund debt; includes `processIncome` (not called from `TransactionService` today)
- `MonthCloseoutService` — month hard-close workflow; dispatches `ClassicCloseoutEngine` or `FamilyPooledCloseoutEngine` from `families.closeout_mode`, then advances / split-debt consolidation / interest; writes `settings_snapshot` + `results_snapshot` on `MonthHardClose`; also provides `getMonthStatus` for month summaries
- `App\Services\Closeout\*` — shared closeout engines (`ClassicCloseoutEngine`, `FamilyPooledCloseoutEngine`), `CloseoutEngineResolver`, `CloseoutAllocationWriter`, `CloseoutTotals`, `CloseoutRulePreviewBuilder`, `CloseoutArtifactReconstructor`, `LegacyCloseoutDataBackfill`, plus `CloseoutMode` / `CloseoutScope` allowed-value helpers
...
- `PlaidMatchingService` — merchant-rule suggestions, ledger/sweep/repayment-group matching, and confirm/dismiss learning; used by `PlaidTransactionSyncService`, `PlaidImportController`, and `PlaidCalibrationService`. Ledger auto-match only considers rows with **null** `plaid_transaction_id` (see `docs/ai/09-known-decisions.md` — confirmed pending→posted Plaid ids can re-queue)

**Read-only controllers:**
- `MonthSummaryController::show` — returns comprehensive month overview (close status, category totals including optional synthetic **Uncategorized Debt Payments** for repayments without a category, member balances, rule preview, **`closeout_preview`**) without modifying data; when **`view_family_expenses`** is on, **`family_category_totals`** omit inter-member debt payments; **open months** dry-run the current closeout mode; **hard-closed months** read **`results_snapshot`** (or reconstruct from ledger artifacts if the snapshot is missing). **`rule_preview`** is the viewer slice of that computation. **`rule_preview.basis.total_expenses`** aligns with **`MonthCloseoutService::expenseTotalTowardRemainingBasis`** on live classic previews (tracked repayments included; remaining-exclusion advances omitted from the basis and settled via fund advances). **`rule_preview.basis.remaining_after_expenses`** is the **signed** post-allocation figure in classic mode (member leftover pool in family pooled mode).
...
| Category | Per `family_id`; each row is **either** income **or** expense (`is_income` XOR `is_expense`, enforced in `StoreCategoryRequest`); `split_default` and `is_necessity_default` are family-shared on the category while `advance_fund_id` + `exclude_from_expense_basis_default` (classic remaining-exclusion only) are stored per-user-per-category in `category_user_defaults` and only applied when `is_expense` is true |
...
**Transaction list (`GET /transactions`):** default responses include only rows **relevant to the authenticated user**: `user_id` matches them, or they have a `TransactionSplit` on the row (so co-participants see others’ split expenses/income they share, but not unrelated family members’ solo transactions). Optional **`view=family`** returns every family transaction in the date range (same eager loads; splits still appear once). Dashboard does **not** pass that param. On the Transactions page that overlay is browse-only for regular members; **`can_manage_family`** users can edit/delete other members’ unlocked rows. For **split** inter-family debt payments, the payer’s debt-payment **expense** is omitted for the **creditor** when they are both the repayment recipient (`debt.creditor_id`) and a split participant on that expense, so they only see the matching **income** leg (one line per payment).
...
| FundRule | Per `user_id` + `fund_id`; personal closeout rules (classic engine, or leftover remaining-base rules in family pooled) |
| FamilyCloseoutRule | Per `family_id`; family-pooled surplus / remaining-after-charity stages only |
| FundMovement | Per `fund_id` + `user_id`; types include `initial_value` (fund start), `allocation`, `borrow`, `repayment`, `closeout_allocation`, `advance_settlement` (advance fund payout at close), `savings_sweep` (manual sweep to external savings; no transaction), `manual_override` (explicit balance set; signed delta; fund history only — excluded from Month Summary Fund In/Out) |
| Debt | Per `family_id`; links `debtor_id` ↔ `creditor_id` (or `fund_id` for fund borrows); may have linked payment transactions; supports optional closeout interest (`interest_enabled`, `interest_rate`, `interest_last_applied_at`, `loan_received_date`) that accrues at hard-close month-end using daily-rate logic and in-month payment dates; payment history appends an `initial_value` entry (principal-only) and, for inter-family running debts, includes overpayment-reversal lineage (`reversed_from_debt_id`) |
