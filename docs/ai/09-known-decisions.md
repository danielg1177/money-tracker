...
`DebtService::payDebt` (used by `POST /debts/pay`) enforces the balance cap only for external debts (`creditor_id` null). When an in-family payment exceeds the remaining balance, the original debt is zeroed and the excess is applied via `DebtService::applyInterFamilyPairNet` in the swapped direction (`description` prefixed with `Reversed from overpayment:` when a new debt is created). That helper nets against any existing opposite-direction confirmed debt first and merges into an existing same-direction reverse debt when present, so the Debts page does not accumulate A→B and B→A open balances for the same pair. The same swing path is used by `TransactionService::createDebtRepaymentExpense` and `createDebtRepaymentReceivedIncome`.
