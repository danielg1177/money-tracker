...
### Expense “sent to a family member” reuses the debt-payment pair
Create-only `transfer_to_user_id` on expense payloads (FAB `TransactionForm` and Plaid confirm / confirm-split) is mutually exclusive with `debt_id`. The server find-or-creates an open personal in-family running debt (payer → recipient, `balance > 0`; never `$0` debts) and then runs `createDebtRepaymentExpense`. Closeout math and family totals already omit those `is_debt_payment` legs. Edit of an existing family-transfer row still uses `debt_id`. Plaid `learnFromConfirmation` does not store `is_debt_payment` / `debt_id` from this path so a just-zeroed debt is not auto-applied on later imports.
...
Hard-close now creates closeout-tagged expense rows (`transactions.is_closeout_initiated=true`) for fund/debt allocations, and title-completion creates a similar row when marked done. Closeout engines exclude these rows from expense-basis calculations during closeout so they cannot recursively alter same-run allocations. **Fund-allocation** closeout expenses are not ordinary spending: Transactions shows them under **Closeout fund movements** (or **Family closeout fund movements** when `closeout_scope=family`), and Dashboard / bank-balance / category totals omit them. Fund balance still increases via `CloseoutAllocationWriter` and the matching `FundMovement` (`closeout_allocation`) still appears in Fund In/Out and fund history.

### Gross-base fund rules net month advances for the “remaining” pool (preview + hard close)
When computing **remaining-after-gross** input for **`allocation_base='remaining'`** rules (and **`GET /month-summary` `rule_preview.basis`** in classic mode), **gross/net-base** rules whose **`destination_type`** is **`fund`** contribute only **`max(0, nominal_allocation − advance expenses tagged to that fund before the rule in rule order)`** toward the amount subtracted from gross income alongside **`total_expenses`**. Advance-tagged expenses are already in **`total_expenses`**; this avoids double-counting the overlap. Debt and title destinations do not use advance netting. Nominal closeout allocations to funds (ledger movements) are unchanged.
...
### Family expense view is a household overlay
`users.view_family_expenses` (Settings toggle) does not change closeout math, owed/split balances, Fund In/Out, or Dashboard. `GET /transactions?view=family` is opt-in so Dashboard stays viewer-scoped. While the overlay is on, Transactions still allows edit/delete of the viewer’s own rows (`user_id` matches). **`can_manage_family`** (head of household or system admin in the family) can also edit/delete other members’ family rows (same closeout / repaid / benefit / debt-payment-income locks). Regular members still see other members’ rows as browse-only. Split transactions already visible to the viewer are not duplicated. Month Summary **`debt_repayments.paid` / `received`** stay viewer-scoped; when the overlay is on, **`family_debt_paid`** is added so family-shared debt payments can show You vs Family contribution totals. **Family expense totals** (Transactions period/day **Family** figures and Month Summary **`family_category_totals`**) omit payments from one family member to another (`is_debt_payment` expenses whose debt has `creditor_id` set). Viewer **You** totals still count those payments as expenses. External and fund debt payments still count as family spending.

### Hard-closed months freeze closeout results
Every new hard close writes `closeout_mode`, `settings_snapshot` (`version` 1 plus rules), and `results_snapshot` on `month_hard_closes`. Allowed mode strings live in `App\Services\Closeout\CloseoutMode` (and `resources/js/support/closeoutMode.js`). Month Summary for a hard-closed month reads that snapshot (label **Closeout Results**). Open months keep a live dry-run from the current `families.closeout_mode`. Existing hard closes that predate snapshots are backfilled once by `LegacyCloseoutDataBackfill` from ledger artifacts (not today’s rules). Any remaining rows without a snapshot still reconstruct on read as **classic**, even if the family later switches to family pooled. Changing the mode toggle or rules does not rewrite `month_hard_closes`, fund balances, or closeout ledger rows. Transactions **Excluded from remaining** pills for a hard-closed month follow that month’s stored `closeout_mode`, not today’s family mode. Undo hard close then re-close uses the **current** mode.

### Family pooled closeout math
`families.closeout_mode=family_pooled` runs `FamilyPooledCloseoutEngine` instead of per-user classic rules. Charity (surplus) uses expenses with **`is_necessity=true`** only; remaining-after-charity subtracts **all** family closeout expenses (necessities and not). Inter-member debt payments are omitted from **family** closeout income and expenses (internal transfers; cash did not leave the household). Leftover weights use **split shares only**, not solo bills. Personal **gross/net** `FundRule`s are skipped; only remaining-base personal rules run on each member’s leftover `member_pool`. Family-rule allocations are attributed to the user who hits Hard Close (`closeout_scope=family`). Zero income or split-spend ≥ income clamps weight to 0; if all weights are 0, leftover stays unallocated. **`exclude_from_expense_basis` has no effect anywhere in family pooled** (math, leftover preview, or UI). Classic remaining-after-expenses uses that flag; `is_necessity` does not change classic remaining.

### Category defaults overwrite Plaid merchant-rule flags
`plaid_merchant_rules` stay per-user and keep category / split / dismiss / confidence. **Necessity** on those rules follows the family's `categories.is_necessity_default` (also overlaid at suggestion time when the rule has a category). **Advance and remaining-exclusion** follow that user's `category_user_defaults`. Saving a category updates every family member's matching **necessity** flags; personal advance / remaining-exclusion are rewritten only for the member who saved. **Apply defaults to bank learning** updates the signed-in user. That is intentional when moving to family pooled: old remaining-exclusion merchants had `is_necessity=false` and would otherwise keep applying. Confirmed ledger history is not rewritten; closed-month auto-created rows are skipped.
...
### `TransactionController` update/destroy are family-scoped, not owner-only
`TransactionController::update` and `destroy` 403 only when the row is neither owned by the auth user **nor** in the same `family_id`. Any family member can therefore mutate another member’s row via the API. The Transactions UI still treats other members’ overlay rows as browse-only **except** for **`can_manage_family`** users (head of household or system admin), who can edit/delete those rows. Closeout, repaid, benefit, and debt-payment **income** rows stay locked in the UI. Soft-close still checks the **transaction owner**, so a family manager cannot change a row whose owner has soft-closed that month.
...
- `database/seeders/CloseoutDemoSeeder.php` is a local-only closeout UI demo (`php artisan db:seed --class=CloseoutDemoSeeder`); it is not invoked by `DatabaseSeeder`
