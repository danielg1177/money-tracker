...
class PlaidImportController extends Controller
{
    ...
    public function correctAutoCreated(CorrectAutoCreatedImportRequest $request, PlaidPendingImport $pendingImport): JsonResponse
    {
        ...
        $this->matchingService->learnFromConfirmation($user->id, $merchantName, [
            ...
            'exclude_from_expense_basis' => (bool) ($validated['exclude_from_expense_basis'] ?? false),
            'is_necessity' => (bool) ($validated['is_necessity'] ?? true),
            'is_split' => (bool) $transaction->is_split,
            'action' => 'categorize',
            'description' => $validated['description'] ?? $transaction->description,
    ...
    private function buildTransactionPayloadFromImportFields(
        ...
        $payload = [
            ...
            'advance_fund_id' => $resolvedAdvanceFundId,
            'exclude_from_expense_basis' => (bool) ($fields['exclude_from_expense_basis'] ?? false),
            'is_necessity' => (bool) ($fields['is_necessity'] ?? true),
        ];
        ...
        try {
            $ledger = DB::transaction(function () use ($pendingImport, $ledgerId, $user): Transaction {
                ...
                $this->matchingService->learnFromConfirmation($user->id, $merchantName, [
                    ...
                    'exclude_from_expense_basis' => (bool) $ledger->exclude_from_expense_basis,
                    'is_necessity' => (bool) $ledger->is_necessity,
