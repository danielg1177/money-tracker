...
function diagonalSplitBar(type) {
 ...
 * @param {{ confirmDelete?: boolean }} [options]
...
export function transactionTypeBarStyle(transaction, options = {}) {
  ...
  if (isSplit) {
    return { background: diagonalSplitBar(transaction?.type) };
  }
 ...
 * @returns {{ background: string }}
 */
export function categoryRowTypeBarStyle(row, categoryType) {
  const type = categoryType === 'income' ? 'income' : 'expense';
  if (row?.is_split) {
    return { background: diagonalSplitBar(type) };
  }
