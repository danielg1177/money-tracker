    ...
    <div class="px-4 mt-4 space-y-4">
      ...
      <section v-if="user?.family_id" class="rounded-xl border border-gray-700 bg-gray-800 p-4">
        ...
        <p class="mt-2 text-xs text-gray-500 leading-snug">
          When this is on, Transactions and View month show everyone’s household activity. You can still edit and delete your own transactions. {{ canManageFamily ? 'As family manager you can also edit and delete other members’ transactions.' : 'Other members’ rows stay browse-only.' }} Your own totals stay visible next to the family combined figures. Split expenses you already see are not listed twice.
        </p>
...
const canManageFamily = computed(() => Boolean(user.value?.can_manage_family || user.value?.canManageFamily));
